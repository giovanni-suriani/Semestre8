...
cross_fold_score = cross_validate(classifier, X_train, y_train, scoring=["f1_macro", 'precision'], cv=10)
f1_score_treino_teste = avaliar_modelo_treino_teste(classifier, X_train, y_train, X_test, y_test)
print(f"score f1  = {cross_fold_score['test_f1_macro'].mean()}")
print(f"Acurácia SVC sem melhorias = {f1_score_treino_teste:.2f}") #exibe a acurácia


from sklearn.svm import LinearSVC
def print_svc_f1_cross_fold(f1_score, C):
  print(f"Cross Fold f1 SVC com melhorias Cross Fold = {f1_score}, C = {C}")
  
def print_svc_f1(f1_score, C):
  print(f"f1 treino_teste SVC com melhorias = {f1_score}, C = {C}")
...
for C_val in linear_svc_parameters['C']:
    ...
    cross_fold_score = cross_validate(classifier, X_train, y_train, scoring=["f1_macro", 'precision'], cv=10)
    f1_score_treino_teste = avaliar_modelo_treino_teste(classifier, X_train, y_train, X_test, y_test)
    print_svc_f1_cross_fold(cross_fold_score["test_f1_macro"].mean(), C_val, 'linear', 'scale')
    print_svc_f1(f1_score_treino_teste, C_val, 'linear', 'scale')
