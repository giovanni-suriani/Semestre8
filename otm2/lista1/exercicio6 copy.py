# simplex_step_by_step_frac_with_cost.py
import sympy as sp
...
def simplex_step_by_step(A, b, c, I0, max_iter=50):
    ...
    for it in range(1, max_iter + 1):
        ...
        # >>>> Aqui: calcula o valor atual do custo
        # Forma uma solução x atual: variáveis básicas têm xI, as não-básicas são zero
        x = sp.zeros(n, 1)
        for idx, i in enumerate(I):
            x[i, 0] = xI[idx, 0]
        custo_atual = (c * x)[0]     # produto linha x coluna
        ...
        ratios = [xi/yi if yi > 0 else sp.oo for xi, yi in zip(xI, yk)]
